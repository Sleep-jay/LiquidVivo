...
class HookEntry : XposedModule() {
    ...
    companion object {
        ...
        const val HOOK_PREFS_NAME = "liquidvivo_hooks"

        /**
         * Restart-SystemUI action key (Long timestamp): the manager app writes it,
         * the module inside the SystemUI process observes the change and kills itself
         * after a short delay. system_server immediately restarts SystemUI so newly
         * installed / enabled module code gets injected without rebooting the phone.
         */
        const val ACTION_RESTART_SYSTEMUI = "action_restart_systemui"
    ...
    /**
     * HotReload 完成回调：框架在目标进程内重新加载了模块 dex 并重新执行
     * onPackageLoaded（新 hook 已挂载）。这里卸载旧 hook，避免新旧挂载叠加
     * 导致双重缩放 / 重复监听。
     */

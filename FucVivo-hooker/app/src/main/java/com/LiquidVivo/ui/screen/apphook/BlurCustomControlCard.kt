...
import com.LiquidVivo.xposed.hooks.systemui.BlurStyleController.BlurScene
...
fun BlurCustomControlCard() {
    ...
    val values = remember {
        mutableStateMapOf<String, Float>().apply {
            ...
            put(
                BlurStyleController.KEY_GLASS_OPACITY,
                if (prefs.contains(BlurStyleController.KEY_GLASS_OPACITY)) {
                    prefs.getInt(
                        BlurStyleController.KEY_GLASS_OPACITY,
                        BlurStyleController.DEFAULT_GLASS_OPACITY,
                    ).toFloat()
                } else {
                    BlurStyleController.DEFAULT_GLASS_OPACITY.toFloat()
                },
            )
            put(
                BlurStyleController.KEY_GLASS_BLUR_RADIUS,
                prefs.getInt(
                    BlurStyleController.KEY_GLASS_BLUR_RADIUS,
                    BlurStyleController.DEFAULT_GLASS_BLUR_RADIUS,
                ).coerceIn(
                    BlurStyleController.MIN_GLASS_BLUR_RADIUS,
                    BlurStyleController.MAX_GLASS_BLUR_RADIUS,
                ).toFloat(),
            )
        }
    }
    ...
    Card(modifier = Modifier.padding(top = 12.dp).fillMaxWidth()) {
        Text(
            text = stringResource(R.string.blur_liquid_glass_title),
            fontSize = 14.sp,
        ...
        Text(
            text = stringResource(R.string.blur_liquid_glass_caption),
            fontSize = 13.sp,
            color = colorScheme.onSurfaceVariantSummary,
            modifier = Modifier.padding(start = 16.dp, end = 16.dp),
        )
        val glassKey = BlurStyleController.KEY_GLASS_OPACITY
        val glassValue = values[glassKey] ?: BlurStyleController.DEFAULT_GLASS_OPACITY.toFloat()
        SliderPreference(
            title = stringResource(R.string.blur_glass_opacity_title),
            summary = stringResource(R.string.blur_glass_opacity_summary, glassValue.roundToInt()),
            value = glassValue,
            onValueChange = { values[glassKey] = it },
            onValueChangeFinished = {
                writeScale(
                    context,
                    glassKey,
                    (values[glassKey] ?: BlurStyleController.DEFAULT_GLASS_OPACITY.toFloat()).roundToInt()
                        .coerceIn(BlurStyleController.MIN_GLASS_OPACITY, BlurStyleController.MAX_GLASS_OPACITY),
                )
            },
            valueRange = BlurStyleController.MIN_GLASS_OPACITY.toFloat()..BlurStyleController.MAX_GLASS_OPACITY.toFloat(),
        )
        val radiusKey = BlurStyleController.KEY_GLASS_BLUR_RADIUS
        val radiusValue = values[radiusKey] ?: BlurStyleController.DEFAULT_GLASS_BLUR_RADIUS.toFloat()
        SliderPreference(
            title = stringResource(R.string.blur_sidebar_radius_title),
            summary = stringResource(R.string.blur_glass_radius_summary, radiusValue.roundToInt()),
            value = radiusValue,
            onValueChange = { values[radiusKey] = it },
            onValueChangeFinished = {
                writeScale(
                    context,
                    radiusKey,
                    (values[radiusKey] ?: BlurStyleController.DEFAULT_GLASS_BLUR_RADIUS.toFloat())
                        .roundToInt()
                        .coerceIn(
                            BlurStyleController.MIN_GLASS_BLUR_RADIUS,
                            BlurStyleController.MAX_GLASS_BLUR_RADIUS,
            ...
            valueRange = BlurStyleController.MIN_GLASS_BLUR_RADIUS.toFloat()..
                BlurStyleController.MAX_GLASS_BLUR_RADIUS.toFloat(),
        )
    }
        ...
        TextButton(
            ...
            onClick = {
                for (key in values.keys) {
                    values[key] = when (key) {
                        BlurStyleController.KEY_GLASS_OPACITY -> BlurStyleController.DEFAULT_GLASS_OPACITY.toFloat()
                        BlurStyleController.KEY_GLASS_BLUR_RADIUS -> BlurStyleController.DEFAULT_GLASS_BLUR_RADIUS.toFloat()
                        else -> BlurStyleController.DEFAULT_SCALE.toFloat()
                    }
                }
